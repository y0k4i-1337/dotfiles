...
alias stv='pipenv run subl --project $(basename $(pwd)).sublime-project'
...
function find_project() {
  ...
  if [[ $PROJECT_ROOT != "/" ]]; then
    ...
    if [[ $SUBL_DIR != "/" ]]; then
      ...
      echo "${PROJECT_NAME}.sublime-project not found or missing .git directory"
      return 1
    fi
  fi

  subl --project $FINAL_DEST
}
